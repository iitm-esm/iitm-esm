...
!<-- esm insertion fice (line no. 404)
    allocate ( Ice % mask     (isc:iec, jsc:jec)       , &
         ...
         Ice % fice           (isc:iec, jsc:jec, km)   , & !Prajeesh:Change: (isd:ied,jsd:jed,km) to (isc:iec,jsc:jec,km) 
...
!--> esm insertion hice_top, hsno_top, flux_au_top, flux_av_top, flux_at_top, flux_aq_top
! flux_asw_top, flux_alw_top, ta1_ocn_top
...
!--> esm insertion flux_au,  flux_av, flux_at, flux_aq
! flux_asw, flux_alw, ta1_ocn
...
!<-- esm insertion
  ...
  subroutine ice_model_end (Ice)
    ...
    !esm insertion Roxy 080312
    deallocate(Ice % fice)
    deallocate(Ice % flux_sw_vis_dir_top, Ice % flux_sw_vis_dif_top)
    deallocate(Ice % flux_sw_nir_dir_top, Ice % flux_sw_nir_dif_top)
    deallocate(Ice % hice_top, Ice % hsno_top)
    !esm insertion ends 
