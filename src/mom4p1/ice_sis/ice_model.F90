...
contains
  ...
  subroutine update_ice_model_slow_dn ( Atmos_boundary, Land_boundary, Ice )
    ...
    Ice, Land_boundary%runoff, Land_boundary%calving,  &
  ...
  subroutine ice_top_to_ice_bottom (Ice, part_size, part_size_uv)
    type (ice_data_type), intent(inout) :: Ice
    real, dimension (isc:iec,jsc:jec,km), intent(in) :: part_size, part_size_uv !->Prajeesh: redefined witih (isc:iec,jsc:jec,:)
    integer                             :: m, n, ii, jj
  ...
  subroutine ice_bottom_to_ice_top (Ice, t_surf_ice_bot, u_surf_ice_bot, v_surf_ice_bot, &
    ...
    real, dimension(isc:iec,jsc:jec) :: sst, tmp, ohf_sice !<-- esm insertion ohf_sice
    real                             :: u, v
    integer                          :: i, j, k, m, n
    logical                          :: sent
...
!<-- esm insertion
    ...
    do j = jsc, jec
       do i = isc, iec
          if( tmp(i,j) > 0.0) then
		  !--> esm insertion taking MIN of ohf_sice and kmelt*quantity
       ...
       do j = jsc, jec
          do i=isc, iec
             if (Ice%ice_mask(i,j,k)) then
                ...
                !if (ts_new<-80.0) then
                !  call error_mesg('ice_model_mod:', 'temperature too low, resetting',WARNING)
                !  ts_new=-80.0
                !endif
