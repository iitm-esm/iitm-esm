...
!   </DATA>
   ...
   integer, pointer :: i_dest(:,:) =>NULL() ! lon dst indices
   integer, pointer :: j_dest(:,:) =>NULL()  ! lat dst indices
   real,    pointer :: warea(:,:) =>NULL()  ! area covered by water per grid cell
   real,    pointer :: larea(:,:) =>NULL()  ! area of land per grid cell

!   <DATA NAME="discharge_runoff" UNITS="kg/m2/s" TYPE="real, pointer" DIM="2">
!     Water discharge_runoff
!   </DATA>
...
!   </DATA>
   ...
   real,    pointer :: discharge_runoff(:,:) =>NULL() ! water discharge
   real,    pointer :: discharge_drainage(:,:) =>NULL() ! snow discharge
...
integer :: id_discharge_runoff, id_discharge_drainage, id_runoff
...
10   continue
  ...
  allocate( &
       ...
       Rivers%discharge_runoff     (Rivers%is:Rivers%ie,Rivers%js:Rivers%je), &
       Rivers%discharge_drainage(Rivers%is:Rivers%ie,Rivers%js:Rivers%je), &
       stat = ierr)
  ...
  Rivers%discharge_runoff      = 0;
  Rivers%discharge_drainage = 0;
       
  module_is_initialized =.TRUE. 


    print *,'rivers routine initialized' 
...
!   </PUBLICROUTINE>
  ...
  deallocate (               &
       ...
       Rivers%warea,         &
       Rivers%larea,         &
       Rivers%discharge_runoff,     &
       rivers%discharge_drainage  )
...
!   </PUBLICROUTINE>
  ...
  id_discharge_runoff     = register_diag_field (                        &
       mod_name, 'discharge_runoff', (/id_lon, id_lat/),                 &
       Time, 'discharge_runoff', 'kg/s', missing_value=-999.0            )
  id_runoff     = register_diag_field (                        &
       mod_name, 'runoff', (/id_lon, id_lat/),                 &
       Time, 'runoff', 'kg/m2/s', missing_value=-999.0            )
  id_discharge_drainage = register_diag_field (                       &
       mod_name, 'discharge_drainage',  (/id_lon, id_lat/),           &
  ...
  id_adis_snow = register_diag_field (                              &
       mod_name, 'discharge_drainage_aw',  (/id_lon, id_lat/), Time,     &
...
! <DIAGFIELDS>
!   <NETCDF NAME="discharge_runoff" UNITS="kg/s">
!     discharge_runoff
!   </NETCDF>
!   <NETCDF NAME="discharge_drainage" UNITS="kg/s">
!     Snow Discharge
!   </NETCDF>
!   <NETCDF NAME="discharge_aw" UNITS="kg/(m2 s)">
!     Water discharge per unit ocean area
!   </NETCDF>
!   <NETCDF NAME="discharge_drainage_aw" UNITS="kg/(m2 s)">
!     Snow discharge per unit ocean area
!   </NETCDF>
...
!   <TEMPLATE>
!     call update_rivers_slow ( rivers, runoff, drainage )
!   </TEMPLATE>

!   <PUBLICROUTINE>
subroutine update_rivers_slow ( rivers, runoff, drainage )

  type(rivers_type), intent(inout) :: rivers         ! river data to update
  real,              intent(in)    :: runoff(:,:)  ! runoff of liquid water, kg/(m2 s)
  real,              intent(in)    :: drainage(:,:)  ! snow runoff,  kg/(m2 s)
  logical :: used
!   </PUBLICROUTINE>  

  call calc_discharge ( rivers, runoff, rivers%discharge_runoff )
  call calc_discharge ( rivers, drainage, rivers%discharge_drainage )
  ! may be it is more effective to exchange both fields at once: less memory-
  ! efficient, though
  rivers%Time = increment_time(rivers%Time, int(rivers%dt_slow), 0)
  call diag_slow ( rivers )
  if (id_runoff > 0) &
     used = send_data (id_runoff, runoff, rivers%Time)
...
!   </PUBLICROUTINE>
  ...
  bnd % discharge_runoff      = rivers % discharge_runoff
  bnd % discharge_drainage = rivers % discharge_drainage
  
  where ( rivers%warea > 0.0 )
     ! adjust partial-ocean-cell discharge_runoff values for area fraction
     ! and recalculate to kg/(m2 s)
     bnd % discharge_runoff      = bnd % discharge_runoff      / rivers%warea
     bnd % discharge_drainage = bnd % discharge_drainage / rivers%warea
  elsewhere
     bnd % discharge_runoff      = 0
     bnd % discharge_drainage = 0
  endwhere
...
!   </PUBLICROUTINE>
  ...
  if (id_discharge_runoff > 0) &
     used = send_data (id_discharge_runoff, rivers%discharge_runoff, rivers%Time)
  if (id_discharge_drainage > 0) &
     used = send_data (id_discharge_drainage, rivers%discharge_drainage, rivers%Time)
  if (id_adis_water > 0) then
     x=0
     where ( rivers%warea > 0.0 ) &
          x = rivers%discharge_runoff      / rivers%warea
     used = send_data (id_adis_water, x, rivers%Time)
  endif
  if (id_adis_snow  > 0) then
     x=0
     where ( rivers%warea > 0.0 ) &
          x = rivers%discharge_drainage / rivers%warea
