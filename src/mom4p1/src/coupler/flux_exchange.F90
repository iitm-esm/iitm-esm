  ...
  ! </DESCRIPTION>
    ...
    use mpp_mod,         only: mpp_npes, mpp_pe, mpp_root_pe, &
         ...
         mpp_send, mpp_recv, mpp_get_current_pelist !<-- esm insertion
          
    use mpp_domains_mod, only: mpp_get_compute_domain, mpp_get_compute_domains, &
             mpp_global_sum, mpp_redistribute, operator(.EQ.), BITWISE_EXACT_SUM
  ...
  !  use sat_vapor_pres_mod, only: compute_qs !esm deletion 18/07/12
  ...
  real, allocatable, dimension(:) :: &
       ...
       ex_q_surf,    &   ! surface temperature for radiation calc, degK
       ex_t_surf_miz,&   ! miz
  ...
  !    call mpp_set_current_pelist(pelist(1:npes_all) )
      call mpp_set_current_pelist() !<-- esm insertion
      deallocate(pelist)
  ...
  !--> esm insertions ex_cice ex_fice,ex_hice,ex_hsno,ex_t_sst,ex_f_m,ex_f_t,ex_f_q, (f_m, f_t, f_q are passed onto
  ! surface_flux in surface_flux.f90
    ! ---- local vars ----------------------------------------------------------
    ...
    real, dimension(lbound(land%t_sst,1):ubound(land%t_sst,1), &
                         lbound(land%t_sst,2):ubound(land%t_sst,2)) :: landmask
    real,dimension(size(Atm%fice,1),size(Atm%fice,2)) :: &
                                  tsurf_atm, tsurf_atm_clim, fice_atm, hice_atm, hsno_atm, &
                                  tice_atm, tocean_atm
    real, dimension( lbound(Ice%t_surf,1):ubound(Ice%t_surf,1),lbound(Ice%t_surf,2):ubound(Ice%t_surf,2), &
                  lbound(Ice%t_surf,3):ubound(Ice%t_surf,3) ) :: tice, tocean
    
    real    :: zrefm, zrefh
    logical :: used
    character(32) :: tr_name ! tracer name
    integer :: tr, n, m ! tracer indices
    integer :: i, ind_flux = 1, iec, isc, jec, jsc
   !--> esm insertion
     integer :: ii,jj
     integer :: arraylen,is,ie,js,je,worldpeset
     logical :: no_sync=.true. 
  !   subroutine and already predefined in coupler_main.f90. IS THIS CORRECT?
  !<-- esm insertion
  ...
  !--> esm insertion
    ...
    Ice%hice_top(:,:,2:)=Ice%h_ice(isc:iec,jsc:jec,2:)
    Ice%hsno_top(:,:,2:)=Ice%h_snow(isc:iec,jsc:jec,2:)
    ...
    tice(:,:,1)=0.
    ex_tice=0.
    call put_to_xgrid (tice, 'OCN', ex_tice, xmap_sfc)
    
    tocean=Ice%t_surf
    tocean(:,:,2:)=0.
    ex_tocean=0.
    call put_to_xgrid (tocean, 'OCN', ex_tocean, xmap_sfc)

    tocean(:,:,:)=1.0
    ex_ofrac=0.
    call put_to_xgrid(tocean, 'OCN', ex_ofrac, xmap_sfc)
    ...
    call get_from_xgrid (tice_atm, 'ATM', ex_tice, xmap_sfc)         !to get actual tice it should be divided by fice, which will be done in gfs
    call get_from_xgrid (tocean_atm, 'ATM', ex_tocean, xmap_sfc)     ! to get actual tocean it should be divided by (1-fice), which will be done in gfs
    
    call mpp_set_current_pelist(pesetnum=worldpeset, no_sync=no_sync)
    
  ! call mpp_send(tsurf_atm,size(tsurf_atm,1)*size(tsurf_atm,2), Atm%coupler_rank)
    call mpp_send(fice_atm,size(fice_atm,1)*size(fice_atm,2), Atm%coupler_rank)
    call mpp_send(hice_atm,size(hice_atm,1)*size(hice_atm,2), Atm%coupler_rank)
    call mpp_send(hsno_atm,size(hsno_atm,1)*size(hsno_atm,2), Atm%coupler_rank)
    call mpp_send(tice_atm,size(tice_atm,1)*size(tice_atm,2), Atm%coupler_rank)
    call mpp_send(tocean_atm,size(tocean_atm,1)*size(tocean_atm,2), Atm%coupler_rank)

    call mpp_recv(Atm%nsw_i, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%nsw_o, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%nlw, arraylen, Atm%coupler_rank) ! Downward longwave, not net long wave
    call mpp_recv(Atm%shflx_i, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%shflx_o, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%lhflx_i, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%lhflx_o, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%lprec, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%fprec, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%drdt, arraylen, Atm%coupler_rank) !Emissivity
    call mpp_recv(Atm%dhdt, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%dqdt, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%uflux, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%vflux, arraylen, Atm%coupler_rank)
    call mpp_recv(Atm%totrunoff, arraylen, Atm%coupler_rank)
    ...
    call some(xmap_sfc, ex_avail, 'OCN')
    ex_t_surf=0.0
    ex_flux_lw=0.0
    ex_drdt_surf = 0.0
    call put_to_xgrid (Ice%t_surf, 'OCN', ex_t_surf, xmap_sfc)

    ex_flux_lw = stefan*ex_t_surf**4  ! (W/m**2)
    ex_drdt_surf = 4*stefan*ex_t_surf**3  ! d(upwardlongwave)/d(surface temperature)
  ...
  subroutine flux_down_from_atmos (Time, Atm, Land, Ice, &
    ...
    type(atmos_data_type), intent(inout) :: Atm
    type(land_data_type),  intent(in) :: Land
    type(ice_data_type),   intent(inout) :: Ice !<-- esm insertion Ice datatype intent has been modified from in to inout
  ...
  !--> esm insertion
    real, dimension(n_xgrid_sfc) :: ex_seawater, ex_seaice, ex_tmp, ex_tmp1
  !<-- esm insertion
  ...
  call put_to_xgrid (Atm%totrunoff,  'ATM', ex_runoff, xmap_sfc)  
  call get_from_xgrid (Land_boundary%runoff,   'LND', ex_runoff,     xmap_sfc)
  Land_boundary%drainage=0.

  sea = 1.0; sea(:,:,1) = 0.0;
  ex_seaice = 0.0
  call put_to_xgrid (sea, 'OCN', ex_seaice, xmap_sfc)
  
  ! Sensible Heat Flux  
  ex_tmp1=0.0
  ex_tmp=0.0
  call put_to_xgrid (Atm%shflx_o, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  where(ex_seawater>0.0) ex_tmp1=ex_tmp
  call put_to_xgrid (Atm%shflx_i, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  where(ex_seaice>0.0) ex_tmp1=ex_tmp
  call get_from_xgrid (Ice_boundary%t_flux,   'OCN', ex_tmp1,    xmap_sfc)

  ! latent Heat Flux  
  ex_tmp1=0.0
  ex_tmp=0.0
  call put_to_xgrid (Atm%lhflx_o, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  where(ex_seawater>0.0) ex_tmp1=ex_tmp
  call put_to_xgrid (Atm%lhflx_i, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  where(ex_seaice>0.0) ex_tmp1=ex_tmp
  call get_from_xgrid (Ice_boundary%q_flux,   'OCN', ex_tmp1, xmap_sfc)

  ! Net Shortwave Flux  
  ex_tmp1=0.0
  ex_tmp=0.0
  call put_to_xgrid (Atm%nsw_o, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  where(ex_seawater>0.0) ex_tmp1=ex_tmp
  call put_to_xgrid (Atm%nsw_i, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  where(ex_seaice>0.0) ex_tmp1=ex_tmp
  call get_from_xgrid (Ice_boundary%sw_flux_vis_dir,  'OCN', ex_tmp1, xmap_sfc)
  Ice_boundary%sw_flux_nir_dir = Ice_boundary%sw_flux_vis_dir*0.25 
  Ice_boundary%sw_flux_vis_dif = Ice_boundary%sw_flux_vis_dir*0.25
  Ice_boundary%sw_flux_nir_dif = Ice_boundary%sw_flux_vis_dir*0.25
  Ice_boundary%sw_flux_vis_dir = Ice_boundary%sw_flux_vis_dir*0.25
 
  ! Net Longwave flux 
  ex_tmp=0.0
  call put_to_xgrid (Atm%nlw, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  ex_tmp1=0.0
  call put_to_xgrid (Atm%drdt, 'ATM', ex_tmp1, xmap_sfc, remap_method=remap_method) !Surface emissivity
  ex_tmp=ex_tmp-(ex_flux_lw*ex_tmp1) ! Now Net longwave
  call get_from_xgrid (Ice_boundary%lw_flux,  'OCN', ex_tmp,   xmap_sfc)
 
  ! dhdt
  ex_tmp=0.0
  call put_to_xgrid (Atm%dhdt, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  call get_from_xgrid (Ice_boundary%dhdt,     'OCN', ex_tmp, xmap_sfc)
  
  ! dedt->dqdt
  ex_tmp=0.0
  call put_to_xgrid (Atm%dqdt, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  call get_from_xgrid (Ice_boundary%dedt,     'OCN', ex_tmp, xmap_sfc)

  ! drdt
  !ex_tmp=0.0
  !call put_to_xgrid (Atm%drdt, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  !call get_from_xgrid (Ice_boundary%drdt,     'OCN', ex_tmp, xmap_sfc)
  call get_from_xgrid (Ice_boundary%drdt,     'OCN', ex_drdt_surf, xmap_sfc)

  ! Wind Stress in X dir
  ex_tmp=0.0
  call put_to_xgrid (Atm%uflux, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  call get_from_xgrid (Ice_boundary%u_flux,     'OCN', ex_tmp, xmap_sfc)

  ! Wind Stress in Y dir
  ex_tmp=0.0
  call put_to_xgrid (Atm%vflux, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  call get_from_xgrid (Ice_boundary%v_flux,     'OCN', ex_tmp, xmap_sfc)
  
  ! Liquid Precipitation
  ex_tmp=0.0
  call put_to_xgrid (Atm%lprec, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  call get_from_xgrid (Ice_boundary%lprec,     'OCN', ex_tmp, xmap_sfc)
  
  ! Frozen Precipitation
  ex_tmp=0.0
  call put_to_xgrid (Atm%fprec, 'ATM', ex_tmp, xmap_sfc, remap_method=remap_method)
  call get_from_xgrid (Ice_boundary%fprec,     'OCN', ex_tmp, xmap_sfc)

  ex_tmp=0.0
  call get_from_xgrid (Ice_boundary%u_star,   'OCN', ex_tmp,    xmap_sfc)
  call get_from_xgrid (Ice_boundary%coszen,   'OCN', ex_tmp,    xmap_sfc)
  call get_from_xgrid (Ice_boundary%p,        'OCN', ex_tmp,       xmap_sfc) ! mw mod
...
!real, dimension(:,:), intent(out) :: runoff_ice, calving_ice
  ...
  integer :: ier
  real, dimension(n_xgrid_runoff) :: ex_disrunoff, ex_calving, ex_runoff_hflx, ex_calving_hflx, ex_drainage
...
if (do_runoff) then
  call put_to_xgrid ( Land%discharge_runoff,      'LND', ex_disrunoff,  xmap_runoff)
  call put_to_xgrid ( Land%discharge_drainage, 'LND', ex_drainage, xmap_runoff)
  call get_from_xgrid (ice_buf, 'OCN', ex_disrunoff,  xmap_runoff)
  Land_Ice_Boundary%runoff = ice_buf(:,:,1);
  call get_from_xgrid (ice_buf, 'OCN', ex_drainage,  xmap_runoff)
...
subroutine flux_up_to_atmos ( Time, Land, Ice, Atmos, Land_Ice_Atmos_Boundary, Land_boundary, Ice_boundary )
!<-- esm insertion Atmos data type in function argument list.
  type(time_type),      intent(in)  :: Time
  type(land_data_type), intent(inout)  :: Land
  type(ice_data_type),  intent(inout)  :: Ice
  type(land_ice_atmos_boundary_type), intent(inout) :: Land_Ice_Atmos_Boundary
  ...
  integer :: isc,iec,jsc,jec 
  integer :: ii !<-- esm insertion

!--> esm insertion
  type(atmos_data_type),  intent(in)  :: Atmos
  real, dimension(n_xgrid_sfc) :: ex_t_sst,ex_tsurf,ex_fice,ex_hice, ex_hsno, &
                                  ex_tice, ex_tocean
  real,dimension(size(Atmos%fice,1),size(Atmos%fice,2)) :: & 
                                  tsurf_atm, tsurf_atm_clim, fice_atm, hice_atm, hsno_atm, &
                                  tice_atm, tocean_atm, ocean_frac
...
!es insertion 13/08/12 ex_dt_q_surf
...
!        call put_to_xgrid ( Land%tr(:,:,:,n), 'LND', ex_tr_surf_new(:,tr), xmap_sfc )
        call put_to_xgrid ( Land%tr(:,:,:,isphum), 'LND', ex_q_surf_new, xmap_sfc )
     endif
!esm insertion 13/08/12
...
!     call get_from_xgrid (Land_Ice_Atmos_Boundary%dt_tr(:,:,n), 'ATM', ex_delta_tr_n(:,tr), xmap_sfc)
     call get_from_xgrid (Land_Ice_Atmos_Boundary%dt_tr(:,:,isphum), 'ATM', ex_delta_q_n, xmap_sfc)
  enddo
!esm inertion 13/08/12
