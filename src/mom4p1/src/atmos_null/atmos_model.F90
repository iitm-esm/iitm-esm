...
use mpp_mod, 	         only : mpp_root_pe, mpp_get_current_pelist, mpp_recv !<-- Prajeesh insertion
use mpp_mod,           only : mpp_set_current_pelist,stdlog, mpp_send !<-- esm insertion
use mpi,               only : MPI_INTEGER !<-- esm insertion
...
type surf_diff_type
  ...
  real, pointer, dimension(:,:)   :: dflux_q => NULL()
  real, pointer, dimension(:,:)   :: delta_t => NULL()
  real, pointer, dimension(:,:)   :: delta_q => NULL()
...
!<PUBLICTYPE >
 type atmos_data_type
     ...
     real, pointer, dimension(:,:) :: q_bot    => NULL() ! temperature at lowest model level
...
integer                       :: coupler_rank !-->esm insertion coupler_rank <--
real, pointer, dimension(:,:) :: nsw_i =>NULL(), &
                                 nsw_o =>NULL(), &
                                 nlw =>NULL(), &
                                 shflx_i =>NULL(), &
                                 shflx_o =>NULL(), &
...
!--> esm insertion
  ...
  integer :: atmos_rootpe, atmos_npes, worldpeset, from_pe
...
call mpp_send(ibuf, 6, atmos_rootpe)

if(mpp_pe().eq.atmos_rootpe) then
  do i = 1, size(Atmos%pelist)
    from_pe = Atmos%pelist(i)
    call mpp_recv(ibuffer((i-1)*6+1:(i-1)*6+6), 6, from_pe)
  enddo
  ...
  write( *, '(/a,i3,a,i3)' )' domain decomposition: ', layout(1), &
     ' X', layout(2)
...
tile = tile_ids(1)
deallocate(tile_ids)
call get_grid_cell_vertices('ATM',tile,glon_bnd,glat_bnd)!<-- esm This call works
!call MPI_barrier(atmos_mpi_comm,ierr)
!print *, 'After get_grid_cell_vertices call'
...
!print *, 'glon_bnd' , glon_bnd(1,1)
!print *, 'glon_bnd' , glat_bnd(1,1)
if(ntile==1) then
...
allocate ( Atmos%q_bot(is:ie,js:je) )
...
allocate(Atmos%nsw_i(is:ie,js:je))
allocate(Atmos%nsw_o(is:ie,js:je))
allocate(Atmos%nlw(is:ie,js:je))
allocate(Atmos%shflx_i(is:ie,js:je))
allocate(Atmos%shflx_o(is:ie,js:je))
allocate(Atmos%lhflx_i(is:ie,js:je))
allocate(Atmos%lhflx_o(is:ie,js:je))
allocate(Atmos%drdt(is:ie,js:je))
allocate(Atmos%dhdt(is:ie,js:je))
allocate(Atmos%dqdt(is:ie,js:je))
allocate(Atmos%uflux(is:ie,js:je))
allocate(Atmos%vflux(is:ie,js:je))
allocate(Atmos%totrunoff(is:ie,js:je))
...
allocate ( Atmos%Surf_diff%dflux_q(is:ie, js:je) )
allocate ( Atmos%Surf_diff%delta_t(is:ie, js:je) )
allocate ( Atmos%Surf_diff%delta_q(is:ie, js:je) ) 
