...
subroutine nphysicsB (Time, Thickness, Dens, rho, T_prog, &
  ...
  taum1 = Time%taum1
  tau   = Time%tau
  print *, 'Test_nphysics: before delqc'
  ...
  enddo
  print *, 'Test_nphysics: after delqc'

  ! time dependent inverse dzwt 
  do k=0,nk
    dzwtr(:,:,k) = 1.0/Thickness%dzwt(:,:,k) 
  enddo
  print *, 'Test_nphysics: before surf_bdlthick exchange'
  ...
  do j=jsc,jec
     do i=isc,iec
        ...
        !surf_bdlthick(i,j) = 50.0	!Roxy esm insertion 270312 
     enddo
  enddo
  call mpp_update_domains(surf_bdlthick(:,:), Dom%domain2d) 
  print *, 'Test_nphysics: surf_bdlthick exchanged'
  do k=1,nk
     do j=jsd,jed
        do i=isd,ied
