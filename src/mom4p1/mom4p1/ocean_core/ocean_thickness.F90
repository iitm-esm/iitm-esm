...
subroutine update_tcell_thickness (Time, Grid, Ext_mode, Dens, Thickness)
  ...
  do j=jsc,jec
     do i=isc,iec
	    ...
	    print *, 'unstable co-ordinates @',i,j
	    print *, 'eta_t(',i+Dom%ioff,',',j+Dom%joff,') = ',Ext_mode%eta_t(i,j,taup1)
	    print *, 'depth(',i+Dom%ioff,',',j+Dom%joff,') = ',Grid%ht(i,j)
	    Ext_mode%eta_t(i,j,taup1)=-Grid%ht(i,j)+10.0 !Roxy esm insertion 280312
	    huge_undulations=.false. !Roxy esm insertion 280312
        endif
     enddo
  enddo
  if(huge_undulations) then 
      print *, 'unstable co-ordinates @',isd, ied, jsd, jed
      call mpp_error(FATAL, &	!esm deletion commented out
