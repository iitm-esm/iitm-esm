...
use   monin_obukhov_mod, only: mo_drag, mo_profile, mo_drag_cpl !<-- esm insertion (mo_drag_cpl)
...
!--> esm insertion
! Module interfaces surface_flux_0d & surface_flux_2d are currently not implemented.
! Required changes for mo_drag_cpl are already done. In future these may be used by
...
logical :: gfs_coef             = .true. !<-- esm insertion
...
!<-- esm insertion line number 371 f_m,       f_t,        f_q,
!</PUBLICROUTINE>
...
! ============================================================================
  ...
  real, intent(in),  dimension(:) :: &
       ...
       f_m,       f_t,        f_q,                           &
       rough_mom, rough_heat, rough_moist,  rough_scale, gust
!<-- esm insertion line number 390 f_m,       f_t,        f_q,
...
!<-- esm insertion line number 632 f_m_0,       f_t_0,        f_q_0,
  ! ---- arguments -----------------------------------------------------------
  ...
  real, intent(out) ::                                                 &
       ...
       w_atm_0,     u_star_0,     b_star_0,    q_star_0,               &
       cd_m_0,      cd_t_0,       cd_q_0
!<-- esm insertion line number 647 f_m_0,       f_t_0,        f_q_0,
  ...
  real, dimension(1) :: &
       ...
       f_m,       f_t,        f_q,                           &
       rough_mom, rough_heat, rough_moist,  rough_scale, gust
!<-- esm insertion line number 665 f_m,       f_t,        f_q,
...
!<-- esm insertion
  ...
  call surface_flux_1d (                                                 &
       ...
       dhdt_atm,  dedq_atm,   dtaudu_atm, dtaudv_atm,                    &
       dt,        land,      seawater, avail  )
!<-- esm insertion line number 710 f_m,       f_t,        f_q,
...
subroutine surface_flux_2d (                                           &
     ...
     dhdt_atm,  dedq_atm,   dtaudu_atm, dtaudv_atm,                    &
     dt,        land,       seawater,  avail  )
!<-- esm insertion line number 748 f_m,       f_t,        f_q,
  ...
  real, intent(in),  dimension(:,:) :: &
       ...
       f_m,       f_t,        f_q,		       	     &
       rough_mom, rough_heat, rough_moist, rough_scale, gust
!<-- esm insertion line number 764 f_m,       f_t,        f_q,
...
!<-- esm insertion line number 785 f_m(:,j),       f_t(:,j),        f_q(:,j),
