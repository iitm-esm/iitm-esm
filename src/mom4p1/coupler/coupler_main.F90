...
!
  ...
  use mpp_mod,		       only: mpp_send !<-- esm insertion			
  ...
  use mpp_domains_mod,         only: mpp_get_global_domain, mpp_global_field !<-- esm insertion
...
! </NAMELIST>
  ...
  integer :: dt_ocean = -1  !<-- esm insertion  oceanic time step


  integer ::atmos_npes=0, ocean_npes=0, ice_npes=0, land_npes=0
  logical :: do_atmos =.true., do_land =.true., do_ice =.true., do_ocean=.true. ! do_irestart ? 
...
!-->  esm insertion
  integer :: coupler_id=0, ocean_id=2, worldpeset
  logical :: no_sync=.true.
  integer :: yy, mm, dd, hh, mns, secs, yym, mmm, ddm
  type(time_type) :: time_nc_combine
  logical :: opened_nc_combine=.false.
!--> esm insertion
...
!--> esm insertion
  call mpp_init(myid=ocean_id, cpl_id=coupler_id, cpl_rank=Atm%coupler_rank)
  write( text,'(a,i4)' )'coupelr rank=', Atm%coupler_rank 
  ...
  slow: do nc = 1, num_cpld_calls
      ...
      if( Atm%pe )then
        ...
        fast: do na = 1, num_atmos_calls
          ...
          !      ---- ice model ----
          if (do_ice) call update_ice_model_fast(Atmos_ice_boundary, Ice )
...
10  call mpp_close(unit)
    ...
    !CODE FOR COUPLER AND MODEL RESTART NEEDS TO GO HERE IF REQUIRED!!!!!!!!!!!  !<-- esm comment
...
!--> esm insertion
       if (pe.EQ.mpp_root_pe() )then
         ibuf(1)=ocean_id
         ibuf(2)=mpp_root_pe()+1
         call mpp_set_current_pelist(pesetnum=worldpeset,no_sync=no_sync)
         call mpp_send(ibuf, 2, Atm%coupler_rank)   !suppose tag=1
         ibuf(2)=size(Atm%pelist)
         call mpp_send(ibuf, 2, Atm%coupler_rank)
         call mpp_send(Atm%pelist+1, ibuf(2), Atm%coupler_rank)
         ibuf(1)=dt_atmos
         ibuf(2)=dt_cpld
         if(ibuf(2) == 0 ) ibuf(2) = dt_ocean
         call mpp_send(ibuf, 2, Atm%coupler_rank)   !suppose tag=1
...
!--- get the time that last intermediate restart file was written out.
    ...
    if(ALL(restart_interval ==0)) then
       Time_restart = increment_date(Time_end, 0, 0, 1, 0, 0, 0)   ! no intermediate restart
       !changed by Prajeesh A G, as increment_date has bug when giving
       !increments as years or months
       ...
       write(*,*) 'restart_interval:',restart_interval
...
!----- determine maximum number of iterations per loop ------
    ...
    num_atmos_calls = Time_step_cpld  / Time_step_atmos
		...
		if (pe.EQ.mpp_root_pe() )then
         call mpp_set_current_pelist(pesetnum=worldpeset,no_sync=no_sync)
         ibuf(1)=num_cpld_calls
         ibuf(2)=num_atmos_calls
         call mpp_send(ibuf, 2, Atm%coupler_rank)   !suppose tag=1
...
!---- ice -----------
    ...
    if( Ocean%is_ocean_pe )then
        call mpp_set_current_pelist(Ocean%pelist)
!---- ocean ---------
        call ocean_model_init( Ocean, Ocean_state, Time_init, Time, dt_ocean ) !Prajeesh: dt_ocean
