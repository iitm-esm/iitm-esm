...
!...................................
!  ---  inputs:
     &     ( nsoil, couple, icein, dt, zlvl, sldpth,                    &
...
!     snwp     - real, snowfall rate (kg m-2 s-1)                  1    !
...
!  ---  inputs:
     ...
     &       slopetyp
      ...
      real (kind=kind_phys), intent(in) :: dt, zlvl, lwdn,              &
...
!  ---  outputs:
      ...
      real (kind=kind_phys), intent(out) :: shdfac, snowh,              &
...
!           into the soil so that prcp1 is set to zero.
      ...
      sn_new=0.0
      if (snwp>0.0) sn_new=sn_new+snwp*dt*0.001 
      if (frzgra) sn_new=sn_new+prcp*dt*0.001
...
!  --- ...  non-glacial land
!           if snow depth=0, set snowcover fraction=0.
        if (sneqv == 0.0) then
          sncovr = 0.0
        else
...
!           effects caused by falling precipitation.
        ...
        if (snwp > 0.0) rr = rr + cpice*snwp/rch
...
!           snowfall striking the gound is =sfctmp (lowest model level air temp).
      ...
      flx1 = 0.0
      if(snwp>0.0) flx1 = flx1 + cpice * snwp * (t1 - sfctmp)
      if (prcp > 0.0) flx1 = flx1 + cph2o1 * prcp * (t1 - sfctmp)
