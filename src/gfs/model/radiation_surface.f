...
!         inputs:                                                      !
!           (slmsk,snowf,sncovr,snoalb,zorlf,coszf,tgndf,tairf,hprif,  !
...
!         inputs:                                                      !
!           (xlon,xlat,slmsk,snowf,sncovr,zorlf,tgndf,tairf,hprif,     !
!            IMAX)                                                     !
!         outputs:                                                     !
!           (sfcemis)                                                  !
!                                                                      !
...
!     tgndf (IMAX)  - ground surface temperature in k                   !
...
!  ---  inputs
     ...
     &       slmsk, snowf, zorlf, coszf, tgndf, tairf, hprif,           &
     &       alvsf, alnsf, alvwf, alnwf, facsf, facwf, fice, tisfc,     &
     &       sncovr, snoalb
...
!     (originally snowf in mm; zorlf in cm)
         ...
         if (nint(slmsk(i)) == 0 .and. tgndf(i) > con_tice) fsno0 = 0.0
         fsno1 = 1.0 - fsno0
...
! --- diffused sea surface albedo
         ...
         if (tgndf(i) >= 271.5) then
            asevd = 0.06
            asend = 0.06
         elseif (tgndf(i) < 271.1) then
            asevd = 0.70
            asend = 0.65
         else
            a1 = (tgndf(i) - 271.1)**2
...
!  ---  check for snow covered area
          ...
          if ( ialbflg==1 .and. nint(slmsk(i))==1 ) then ! input land area snow cover
            ...
            if ( snowf(i) > my_zero ) then
              ...
              hrgh  = min(1.0, max(0.20, 1.0577-1.1538e-3*hprif(i) ) )
              fsno0 = asnow / (argh + asnow) * hrgh
              if (nint(slmsk(i)) == 0 .and. tgndf(i) > 271.2)           &
     &                               fsno0=my_zero
              fsno1 = 1.0 - fsno0
              sfcemis(i) = sfcemis(i)*fsno1 + emsref(8)*fsno0
            endif
