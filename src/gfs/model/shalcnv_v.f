...
! hchuang code change mass flux output
     &,                    ud_mf(ix,km),dt_mf(ix,km)
!
      integer              i, indx, jmn, k, kk, latd, lond, km1
      integer              kpbl(im)
!
...
!
     ...
     &                     ktcon(im), kbm(im), kmax(im)
!
      real(kind=kind_phys) delhbar(im), delq(im),   delq2(im),
...
c   but limited to the level of 700 mb
c
      do i = 1, im
        flg(i) = cnvflg(i)
...
!!
c
c  compute updraft mass flux for the cloud layers
c
      do k = 2, km1
        do i = 1, im
          if(cnvflg(i)) then
