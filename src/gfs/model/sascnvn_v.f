     ...
     &     q1,t1,u1,v1,rcs,cldwrk,rn,sn,kbot,ktop,kcnv,slimsk,
...
c    &            .743,.813,.886,.947,1.138,1.377,1.896/
      real(kind=kind_phys) tf, tcr, tcrf, zcons1, vtmpc2
      parameter (tf=233.16, tcr=263.16, tcrf=1.0/(tcr-tf))
      real (kind=kind_phys), parameter:: cpv=1869.46,  ! specific heat of water vapour at con P 
     &   cpd=1005.46    ! specific heat of dry air at con p
!
c-----------------------------------------------------------------------
!
      km1 = km - 1
...
!-> prajeesh: added snow fall in convetive precipitation
              if(t1(i,k).gt.t0c) then
              	rntot(i)=rntot(i)+rain*xmb(i)*0.001*dt2
                if((sn(i).gt.0.0).and.t1(i,k).gt.t0c2) then
									zfac=zcons1*(1.+vtmpc2*q1(i,k))
     &                        *(prsl(i,k)-prsl(i,k+1))*1000.
                  zsnmlt=min(sn(i),zfac*(t1(i,k)-t0c2))
                  sn(i)=sn(i)-zsnmlt
                  rntot(i)=rntot(i)+zsnmlt
                endif
              else
               sn(i)=sn(i)+rain*xmb(i)*0.001*dt2
              endif
!<- prajeesh: added snow fall in convetive precipitation
            endif
          endif
...
!-> prajeesh: added snow fall in convetive precipitation
              dp = 1000. * del(i,k)
              if(t1(i,k).gt.t0c) then
              	rn(i)=rn(i)+rain*xmb(i)*0.001*dt2
                if((sn(i).gt.0.0).and.t1(i,k).gt.t0c2) then
									zfac=zcons1*(1.+vtmpc2*q1(i,k))
     &                        *(prsl(i,k)-prsl(i,k+1))*1000.
                  zsnmlt=min(sn(i),zfac*(t1(i,k)-t0c2))
                  sn(i)=sn(i)-zsnmlt
                  rn(i)=rn(i)+zsnmlt
                  t1(i,k)=t1(i,k)-(zsnmlt*1000*(g/dp)*(alf/cp))
                endif
              else
               sn(i)=sn(i)+rain*xmb(i)*0.001*dt2
               if(sn(i).ne.sn(i)) stop "sn(i) is nan"
               t1(i,k)=t1(i,k)+(rain*xmb(i)*dt2*(g/dp)*(alf/cp))
              endif
!<- prajeesh: added snow fall in convetive precipitation
            endif
...
c
          ...
          if(sn(i).lt.0..and..not.flg(i)) sn(i) = 0.
          if(rn(i)+sn(i).le.0.) then
            rn(i) = 0.
            sn(i) = 0.
          else
            ktop(i) = ktcon(i)
...
!
      do k = 1, km
        do i = 1, im
          if (cnvflg(i) .and. rn(i)+sn(i).gt.0.) then
            if (k.gt.kb(i).and.k.le.ktcon(i)) then
...
c
      do k = 1, km
        do i = 1, im
          if(cnvflg(i).and.rn(i)+sn(i).le.0.) then
            if (k .le. kmax(i)) then
              t1(i,k) = to(i,k)
              q1(i,k) = qo(i,k)
              u1(i,k) = uo(i,k)
